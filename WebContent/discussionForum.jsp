    ...
    <div class="art-sheet">
        ...
        <div class="art-sheet-cc">
			<table style="width: 100%; overflow:auto;">
				...
				<tbody>
					<%! Queue<DiscussionForum> queue; %>
					<%! DiscussionForum discussionForum; %>
					<%! @SuppressWarnings("unchecked") %>
			
					<% 
					   	queue = (Queue<DiscussionForum>) request.getAttribute("topics");
						while (queue.isEmpty() == false)
						{
							discussionForum  = queue.poll();
					%>
					<tr>
						<td><%=discussionForum.getDateTime().toString() %></td>
						<td><a href="DiscussionForumCommentsService?discussionid=<%=discussionForum.getDiscussionId().toString() %>"><%=discussionForum.getTitle() %></a></td>
						<td><%=discussionForum.getPostedBy() %></td>
					</tr>
					<% } %>
				</tbody>
			</table>
			<br/><br/>
			<form name="mySearch">
<input type="button" style="text-align: center;" name="Post Topic" value="Post Topic" onclick="window.location.href='postDiscussion.jsp'"/>
</form>
