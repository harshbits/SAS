...
<%@ page import="sas.data.storage.objects.DiscussionForum" %>
<%@ page import="sas.data.storage.objects.DiscussionForumComments" %>
...
<div class="art-header">
    ...
    <div class="art-sheet">
        ...
        <div class="art-sheet-cc">
			...
			<%! Queue<DiscussionForumComments> queue; %>
			<%! DiscussionForumComments discussionForumComments; %>
			<%! DiscussionForum discussionForum; %>    
			<%! @SuppressWarnings("unchecked") %>
			<% 
				discussionForum = (DiscussionForum) request.getAttribute("topic");
			%>   
			<div class="question">
				<h6><%=discussionForum.getTitle() %></h6>
				
					<pre><%=discussionForum.getDescription() %></pre>
			</div>
			<hr/>
			<p style="font-size: 20px;"><b>Comments</b></p>
			
			<form name="myForm" action="DiscussionForumCommentsAddService" method="post" onsubmit="return validateDiscussionForumAddComments()">
			    <input type="hidden" id="discussionid" name="discussionid" value=<%=discussionForum.getDiscussionId() %>></input>
			...
			<% 
				queue = (Queue<DiscussionForumComments>) request.getAttribute("comments");
				while (queue.isEmpty() == false)
				{
					discussionForumComments  = queue.poll();
			%>        
        
				<li>
					<p><%=discussionForumComments.getPostedBy() + " commented on " + discussionForumComments.getDateTime().toString() + ":" %></p><br/>
					<pre><%=discussionForumComments.getComments() %></pre>
				</li>
